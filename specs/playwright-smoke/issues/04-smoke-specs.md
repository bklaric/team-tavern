...
The bot check above, and smoke check 2 in `spec.md`, both name `/games/<handle>`
and both ask for a listing, so one of the two has to give. Measured on a cold
test stack: `/games/valorant` prerenders 14143 bytes with the right `<title>`
and no nickname; `/games/valorant/players` prerenders with `ValorantTester` and
