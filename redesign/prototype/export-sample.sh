#!/bin/bash
# Exports sample posts for the prototype feed from the development database,
# which holds the production dump, into data/<handle>.js:
#   ./redesign/prototype/export-sample.sh valorant valheim
# The development stack's postgres container has to be running. data/ is
...
mkdir -p "$here/data"
for handle in "$@"; do
    json="$(docker exec -i postgres psql -U bklaric -d team_tavern -At -v ON_ERROR_STOP=1 \
