...
import TeamTavern.Session.Start.ReadModel (Nonce)
...
confirmEmailParameters { playerId, nonce } =
    [show $ unwrap playerId, unwrap nonce] <#> QueryParameter

confirmEmail :: forall querier errors. Querier querier =>
    ConfirmEmailModel -> querier -> Async (ConfirmEmailError errors) Unit
confirmEmail model @ { playerId, nonce } querier = do
    result <- querier
