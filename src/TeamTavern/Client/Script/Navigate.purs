...
import Web.HTML.Location (hostname, setHref)
import Web.HTML.Window (history, location)
import Web.HTML.Window as Window
import Web.UIEvent.MouseEvent (MouseEvent)
import Yoga.JSON (class WriteForeign, write)

pathAndQueryStaging :: ∀ effect. MonadEffect effect => String -> effect String
pathAndQueryStaging pathAndQuery = liftEffect do
    hostname <- window >>= location >>= hostname
    let pathAndQuerySplit = split (Pattern "?") pathAndQuery
    let stagingOrLocalhost = hostname == "staging.teamtavern.net" || hostname == "localhost"
    pure case pathAndQuerySplit, stagingOrLocalhost of
        [path], true -> path <> "?nntestads=staging"
        [path, query], true -> path <> "?" <> query <> "&nntestads=staging"
        _, _ -> pathAndQuery

navigate :: ∀ effect state. MonadEffect effect =>
    WriteForeign state => state -> String -> effect Unit
navigate state path = liftEffect do
    path' <- pathAndQueryStaging path
    window
        >>= history
